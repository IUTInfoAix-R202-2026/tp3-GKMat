...
/**
 * Contrôleur de la pierre angulaire MVC (parcours P3 - vérification d'une nuit
 * de capture par
 * échantillonnage).
 *
 * <p>
 * L'instance possède son propre modèle ({@link NuitVerification}). Le FXML
 * s'occupe de la
...
public class QualificationController {
  ...
  @FXML
  private TableView<Sequence> tableView;

  @FXML
  private TableColumn<Sequence, LocalTime> colHorodatage;

  @FXML
  private TableColumn<Sequence, Number> colFrequence;

  @FXML
  private TableColumn<Sequence, Number> colDuree;

  @FXML
  private TableColumn<Sequence, String> colStatut;
  ...
  @FXML
  private Label labelLecture;

  @FXML
  private ChoiceBox<String> choiceBoxVerdict;

  @FXML
  private TextArea zoneCommentaire;

  @FXML
  private Label labelVerdictGlobal;

  private final NuitVerification nuit = NuitVerification.genererJeu(10);

  /**
   * Méthode appelée automatiquement après injection des champs {@code @FXML}.
   * Tout le câblage MVC
   * se passe ici.
   */
  @FXML
  private void initialize() {
    ...
    tableView.getSelectionModel().selectedItemProperty().addListener(
        (obs, ancien, nouveau) -> {
          if (nouveau == null) {
            ...
            labelSelection.setText(String.format("Séquence %s - %.1f kHz",
    ...
    boutonEcouter.disableProperty().bind(
        tableView.getSelectionModel().selectedItemProperty().isNull());
    ...
    labelVerdictGlobal.textProperty().bind(
  ...
  /**
   * Action du bouton « Écouter ». Lecture audio simulée : statut → "Écoutée" +
   * label éphémère.
   */
  @FXML
  ...
  /**
   * Action du bouton « Enregistrer le verdict ». Écrit le verdict choisi dans le
   * modèle.
   */
  @FXML
  ...
  /**
   * Exposé pour les tests : permet de vérifier l'état du modèle après actions sur
   * la vue.
   */
