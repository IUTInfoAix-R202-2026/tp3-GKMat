 ...
 *
 * <p>
 * Affiche un titre, un sous-titre dynamique « N site(s) déclaré(s) » et un
 * bouton « + Nouveau
 * site ». L'action déclenchée par le bouton est fournie de l'extérieur par le
 * contrôleur parent via
 * {@link #setActionNouveauSite(Runnable)} : ce contrôleur ne connaît <b>pas</b>
 * directement la
...
public class EnTeteController {
  ...
  @FXML
  private Button boutonNouveauSite;

  private Runnable actionNouveauSite = () -> {
  };

  /**
   * Méthode invoquée automatiquement par {@link javafx.fxml.FXMLLoader} après
   * injection des
   * composants. On branche ici le clic du bouton sur l'action externe et on
  ...
  /**
   * Permet au contrôleur parent de fournir l'action à exécuter quand
   * l'utilisateur clique sur le
   * bouton « + Nouveau site ». Le parent y branchera typiquement l'ajout d'une
   * carte dans la liste.
   */
