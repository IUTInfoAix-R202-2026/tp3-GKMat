 ...
 *
 * <p>
 * Démontre le motif d'<b>injection automatique des sous-contrôleurs</b> : pour
 * chaque {@code
 * <fx:include source="..." fx:id="xxx"/>}, FXMLLoader injecte deux champs dans
 * le contrôleur parent
 * :
 *
 * <ul>
 * <li>{@code @FXML private Node xxx;} - le nœud racine de la sous-vue
 * <li>{@code @FXML private XxxController xxxController;} - le contrôleur
 * associé
 * </ul>
 *
 * <p>
 * Le suffixe {@code Controller} (figé par convention dans FXMLLoader) est
 * obligatoire pour que
 * l'injection fonctionne. Cela permet au contrôleur parent d'orchestrer les
 * sous-contrôleurs sans
 * qu'ils aient besoin de se connaître entre eux.
 */
public class VueAccueilController {

  @FXML
  private EnTeteController enTeteController;

  @FXML
  private ListeSitesController listeSitesController;

  /**
   * {@link javafx.fxml.FXMLLoader} appelle {@code initialize()} sur le parent
   * <b>après</b> avoir
   * initialisé tous les sous-contrôleurs. C'est donc ici qu'on les fait
   * dialoguer.
   */
  @FXML
