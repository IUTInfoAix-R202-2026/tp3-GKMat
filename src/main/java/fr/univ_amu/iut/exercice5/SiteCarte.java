...
/**
 * Composant graphique réutilisable représentant une carte de site de suivi
 * VigieChiro.
 *
 * <p>
 * Cette classe illustre le pattern <b>{@code fx:root}</b> : le même objet sert
 * à la fois de
 * <i>racine</i> visuelle (le {@link HBox}) et de <i>contrôleur</i> (les
 * {@code @FXML} sont injectés
 * sur ses propres champs). C'est le moyen standard en JavaFX pour transformer
 * une vue FXML en
 * composant graphique instanciable comme n'importe quel autre Node :
 * {@code new SiteCarte()}.
 *
 * <p>
 * <b>Lien avec la SAE 2.01</b> : cette carte correspond à la maquette M-Sites
 * du brief. Sur le
 * tableau de bord, chaque site déclaré par l'utilisatrice (Marie, Karim,
 * Samuel) est rendu sous
 * forme d'une telle carte, avec un <b>badge de fraîcheur</b> coloré qui indique
 * d'un coup d'œil
 ...
 * <li>{@code badge-cold} (gris) si dernier passage &gt; 30 jours, ou jamais
 * (-1)
 * </ul>
 *
 * <p>
 ...
 * <li>{@link FXMLLoader#setRoot(Object)} et
 * {@link FXMLLoader#setController(Object)} dans le
 * constructeur Java
 * <li>exposition des champs sous forme de {@link StringProperty} /
 * {@link IntegerProperty}
 * observables (style JavaBeans : {@code numeroCarreProperty()},
 * {@code getNumeroCarre()},
 * {@code setNumeroCarre()})
 * <li>réaction à un changement de propriété via un {@code ChangeListener} pour
 * mettre à jour la
 * classe CSS du badge
 * </ul>
 */
public class SiteCarte extends HBox {
  ...
  private final IntegerProperty nombrePassages = new SimpleIntegerProperty(this, "nombrePassages", 0);

  /**
   * Nombre de jours depuis le dernier passage importé. La valeur {@code -1}
   * signifie "jamais".
   */
  private final IntegerProperty joursDepuisDernierPassage = new SimpleIntegerProperty(this, "joursDepuisDernierPassage",
      -1);

  /**
   * Construit le composant en chargeant son FXML dans cet objet (qui joue à la
   * fois le rôle de
  ...
  /**
   * Méthode invoquée automatiquement par {@link FXMLLoader} une fois que les
   * champs {@code @FXML}
   * ont été injectés. C'est ici qu'on relie les labels aux propriétés et qu'on
   * installe l'écouteur
  ...
  private void initialize() {
    ...
    joursDepuisDernierPassage.addListener(
        (obs, oldVal, newVal) -> majBadge(newVal.intValue()));
    // courante pour
  ...
  /**
   * Met à jour le texte et la classe CSS du badge en fonction du nombre de jours
   * écoulés depuis le
   * dernier passage. La valeur {@code -1} est traitée comme "aucun passage jamais
   * importé".
   */
